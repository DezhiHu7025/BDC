...
namespace Common.ActionFilter
{
    ...
    {
        private static ILog log = LogManager.GetLogger(typeof(CheckSessionFilterAttribute));
                    ...
                    {    
                        ...
                        if (httpcontext.Request.Cookies[Kcis.Models.Config.WebURL] != null)
                        {
                            ...
                            if (!"Error".Equals(newUser.Status))
                            {
                                ...
                                //if (!newUser.SourceType.ToUpper().Equals("A"))
                        ...
                        {
                            log.Debug("Session已經消失, 且Cookie也消失!");   
                            Logon(filterContext);
                        }
        ...
        private void Logon(ActionExecutingContext filterContext)
        {
            ...
            RouteValueDictionary dictionary = new RouteValueDictionary (new {controller = "Account", action = "LogIn",returnUrl = filterContext.HttpContext.Request.RawUrl });
